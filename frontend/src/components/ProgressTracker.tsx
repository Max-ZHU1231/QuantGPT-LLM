    ...
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      {expression && (
        <div className="mt-4 flex items-start justify-between gap-3">
          ...
          {isRunning && onCancel && (
            <button
              onClick={onCancel}
              className="shrink-0 flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-medium text-gray-500 bg-gray-50 border border-gray-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
            >
      ...
      {!expression && isRunning && onCancel && (
        <div className="mt-4 flex justify-end">
          <button
            onClick={onCancel}
            className="flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-medium text-gray-500 bg-gray-50 border border-gray-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
          >
            <X className="h-3.5 w-3.5" />
            取消
          </button>
      ...
      {isCancelled && (
        <div className="mt-4 px-3 py-2 rounded-lg bg-orange-50 border border-orange-100">
          <p className="text-sm text-orange-600">已取消回测</p>
        </div>
      )}
    </div>
  );
