"""LLM 最小改动候选生成（DeepSeek OpenAI-compatible Chat API）。

隶属于因子流水线包 ``quantgpt.factor_pipeline``；REST 见 ``minimal_edits_routes``。
"""
...
from .feature_flags import pipeline_llm_audit_enabled
...
def _candidate_row(batch_id: str, seed_id: str, item: dict) -> EditCandidate:
    raw_summary = item.get("edit_summary")
    edit_summary: dict[str, Any]
    if isinstance(raw_summary, dict):
        edit_summary = raw_summary
    else:
        edit_summary = {"edits": [], "edit_direction": ""}
    edits_raw = edit_summary.get("edits")
    edits = edits_raw if isinstance(edits_raw, list) else []
    exp_raw = item.get("expected_impact")
    exp_imp: dict[str, Any] = exp_raw if isinstance(exp_raw, dict) else {}
...
async def generate_minimal_edits_for_seed(
    ...
    *,
    ...
    try:
        ...
        if pipeline_llm_audit_enabled():
            ph = hashlib.sha256((system + "\n---\n" + user_msg).encode()).hexdigest()
            preview = (raw or "")[:8000]
            await write_audit_event(
                db,
                user_id=user_id,
                event_type="minimal_edit_llm_audit",
                entity_type="generation_batch",
                entity_id=batch.id,
                payload={
                    "prompt_hash_sha256": ph,
